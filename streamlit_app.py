...
def main():
    ...
    with st.expander("说明", expanded=False):
        st.markdown("""
        **总体说明：**
        本系统以参考文献的DOI号为首要识别依据。若文献存在DOI号，系统将优先以此自动调取数据库中的对应信息，并与作者提供的题目、发表年份等进行交叉比对，以综合核验文献的真实性；若文献无DOI号，则直接依据作者提供的文献信息进行数据库匹配与核验。
        
        **使用方式：**
        1. **输入稿件编号：** 在指定位置输入来稿编号或稿件识别码。
        2. **录入参考文献：** 在输入框中粘贴参考文献列表。每行仅限一条，参考文献可带数字编号（如 1、[1]、(1) 等）或不带编号，系统均可自动识别。
        3. **启动处理：** 点击「开始处理」按钮，系统将自动进行核验。
        4. **查看报告：** 处理完成后，点击下载HTML格式的报告，并使用浏览器直接打开即可查看。
        5. **清空缓存：** 每条参考文献的处理结果将暂存24小时。由于本系统需调取外部数据库，网络不稳定可能导致输出结果异常。如遇报错，请先点击“清空缓存”，再进行核查以获取最新数据。
        
        **输出结果说明：** 6-10项需查看HTML报告详情后进行人工复核，确有问题则需请作者修改。
        1. **总项：** 查询参考文献总数
        2. **匹配成功：** 数据库中成功匹配到的、与作者提供信息完全一致的文献数量及占比
        3. **有DOI：** 可检索到DOI号的文献数量及占比
        4. **近5年：** 属于近5年内发表的文献数量及占比
        5. **近3年：** 属于近3年内发表的文献数量及占比
        6. **不适合引用：** 更正、被更正、撤稿、被撤稿文章的数量及占比
        7. **AI无法判断：** 该文献结构存疑，本系统无法识别其是否为网页、预印本、专利、书籍、期刊、会议论文
        8. **DOI不符：** 通过DOI号检索到的文献信息与作者提供的题目等信息不一致
        9. **DOI重复：** 两篇或多篇文献对应同一个DOI号
        10. **可能重复：** 通过比对两篇或多篇参考文献的题目、作者信息、期卷号、页码等，若相似度超过80%，视为可能重复文献
